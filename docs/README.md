...
| **Minggu 4: Hari 22-30**| • Paired bootstrap CI & kurva final<br>• Naskah laporan & artikel v0.8<br>• Freeze code & materi bimbingan | **DALAM PROGRES** | • Draf panduan: `Rencana-eksperimen-bimbingan/PANDUAN_MEETING_PEMBIMBING.md`<br>• Catatan progres: `Rencana-eksperimen-bimbingan/CATATAN_PROGRES_BIMBINGAN.md` |
...
```
                              ALUR PIPELINE DSIC-2706
                              
  [ Audio Mentah ] -> [ Standardisasi 32kHz ] -> [ Partisi Recordist-Disjoint ]
                                                               |
        +------------------------------------------------------+
        |                                                      |
        v                                                      v
  [ Gallery Set ]                                        [ Query Set ]
  (42 Perekam)                                           (29 Perekam)
        |                                                      |
        v                                                      v
  [ Ekstraksi Fitur ]                                    [ Injeksi Derau SNR ]
  (MFCC / PANNs / Bio)                                   (Clean s/d -5 dB)
        |                                                      |
        +--------------------------+---------------------------+
                                   |
                                   v
                      [ Cosine Similarity Matrix ]
                                   |
                                   v
                      [ Ranking & Evaluasi mAP@10 ]
```

---

### Q1: Apa itu Metode *Recordist-Disjoint* dan Mengapa Wajib Digunakan?
* **Pengertian Sederhana:**
  * Dalam dataset audio Xeno-Canto, setiap rekaman diunggah oleh orang tertentu (*recordist*). Contoh: Rekaman A diunggah oleh "Budi", Rekaman B oleh "Siti".
  * Metode *Recordist-Disjoint* adalah aturan ketat di mana **orang yang merekam audio di data Galeri (referensi) TIDAK BOLEH muncul sama sekali di data Query (pengujian)**.
* **Mengapa Sangat Wajib? (Fenomena *Clever Hans Effect* pada AI):**
  * Setiap perekam menggunakan mikrofon yang berbeda (misal mic HP murah vs mic shotgun profesional), teknik perekaman berbeda, dan lokasi lingkungan yang khas. Perbedaan ini meninggalkan "jejak derau mikrofon" (*microphone fingerprint*).
  * Jika rekaman "Budi" ada di galeri burung Madu dan juga di query burung Madu, model AI yang pintar akan curang: **AI mencocokkan suara desis mikrofon si Budi, bukan suara burungnya!**
  * Akibatnya nilai akurasi di atas kertas tampak tinggi (95%), tetapi saat alat dipasang di kampus ITERA alat tersebut langsung rusak/gagal total karena mikrofon si Budi tidak ada di ITERA.
* **Cara Kerjanya:**
  * Seluruh data dikelompokkan berdasarkan `recordist_id`.
  * Algoritma graf membagi perekam ke dalam dua kelompok terpisah secara global sehingga $\mathcal{R}_{\text{Gallery}} \cap \mathcal{R}_{\text{Query}} = \emptyset$.

---

### Q2: Apa itu *Data Leakage* (Kebocoran Data) dan Bagaimana Cara Memverifikasi *Zero Leakage*?
* **Pengertian Sederhana:**
  * *Data Leakage* terjadi jika ada secuil pun informasi dari data uji (Query) yang bocor atau pernah "diintip" oleh sistem saat proses pembentukan data referensi (Gallery).
* **Bentuk Kebocoran yang Sering Terjadi:**
  1. *File Leakage:* File audio yang sama dipakai di query dan galeri (misal potongan detik 0-5 di galeri, detik 5-10 di query).
  2. *Recordist Leakage:* Perekam yang sama ada di kedua sisi.
* **Cara Memverifikasi *Zero Leakage* Secara Matematis & Kode:**
  * Secara matematis, irisan himpunan perekam harus merupakan himpunan kosong:
    $$\text{Overlap} = |\mathcal{R}_{\text{Gallery}} \cap \mathcal{R}_{\text{Query}}| = 0$$
  * Di dalam kode program (`tests/integration/test_split_leakage.py`), kita melakukan pengujian logika Python:
    ```python
    gallery_rec = set(df_gallery["recordist_id"])
    query_rec = set(df_query["recordist_id"])
    assert len(gallery_rec.intersection(query_rec)) == 0, "Ada kebocoran perekam!"
    ```
  * Jika perintah `python run_tests.py` dijalankan dan memberikan hasil `PASSED`, artinya matematis terbukti 100% bebas kebocoran.

---

### Q3: Bagaimana MFCC (Mel-Frequency Cepstral Coefficients) Bekerja dari Sinyal Suara Menjadi Angka?
* **Pengertian Sederhana:**
  * Komputer tidak bisa langsung memahami suara dari file audio. Suara adalah getaran tekanan udara dari waktu ke waktu. MFCC adalah teknik klasik pemrosesan sinyal digital untuk mengekstrak "sidik timbre/warna suara" menjadi 20 angka ringkas.
* **Langkah-Langkah Pemrosesan MFCC:**
  1. **Pre-emphasis:** Memperkuat frekuensi tinggi audio agar energinya seimbang dengan frekuensi rendah.
  2. **Framing & Windowing:** Sinyal suara yang panjang dipotong menjadi jendela-jendela kecil (misal 25 milidetik) menggunakan fungsi *Hamming Window* agar batas potongan suara tidak patah tajam.
  3. **Fast Fourier Transform (FFT):** Mengubah gelombang suara dari domain waktu (amplitudo vs detik) menjadi domain frekuensi (spektrum frekuensi mana saja yang bergetar keras).
  4. **Mel Filterbank:** Telinga manusia dan hewan tidak mendengar secara linier. Kita lebih sensitif membedakan frekuensi 500 Hz vs 1000 Hz dibanding 10.000 Hz vs 10.500 Hz. Skala Mel memetakan frekuensi Hertz ke skala biologis:
     $$m = 2595 \log_{10}\left(1 + \frac{f}{700}\right)$$
  5. **Logarithm:** Mengambil nilai logaritma dari energi filterbank untuk meniru persepsi kenyaringan suara manusia (desibel).
  6. **Discrete Cosine Transform (DCT):** Mengompres spektrum log-mel menjadi koefisien ortogonal yang tidak saling berkorelasi. Biasanya diambil 20 koefisien teratas sebagai fitur MFCC.
* **Mengapa MFCC Mudah Hancur oleh Derau?**
  * MFCC murni mengukur energi fisik frekuensi. Begitu ada suara angin kencang (derau frekuensi rendah) atau desis hujan (derau frekuensi tinggi), bentuk spektrum frekuensinya langsung hancur total. MFCC tidak memiliki "otak/pemahaman konteks" bahwa kicauan burung masih ada di balik derau tersebut.

---

### Q4: Perbaikan Bug Kalibrasi Ambang Batas ($\tau$) — Apa yang Terjadi dan Bagaimana Solusinya?
* **Masalah Awal (Bug):**
  * Pada pengujian deteksi spesies tak dikenal (*open-set rejection*), ambang batas kemiripan $\tau$ sempat bernilai anomali $\tau = 0.0000$ dengan False Positive Rate (FPR) sebesar $100\%$. Artinya, sistem menganggap semua suara katak, serangga, dan bising mesin sebagai burung!
* **Penyebab Terjadinya Bug:**
  * Algoritma sebelumnya menggunakan pencarian grid linier kasar (*coarse grid search*), misalnya melompat setiap 0.05 (0.00, 0.05, 0.10, ..., 0.95, 1.00).
  * Pada fitur MFCC yang telah dinormalisasi L2, nilai kemiripan kosinus (*Cosine Similarity*) antar vektor terkumpul sangat rapat di rentang 0.98 hingga 0.99.
  * Lompatan grid kasar tersebut melewatkan titik kritis di 0.985–0.995, sehingga perhitungan optimasi gagal menemukan titik potong dan terjatuh ke nilai default minimum (0.0000).
* **Solusi Perbaikan:**
  * Grid search kasar dihapus dan diganti dengan analisis **Kurva ROC Empiris** (*Receiver Operating Characteristic*).
  * Sistem mengumpulkan seluruh nilai kemiripan aktual yang benar-benar muncul pada data kalibrasi, mengurutkannya, lalu menghitung metrik **Youden's Index ($J$)**:
    $$J = \text{TPR} - \text{FPR} = \text{True Positive Rate} - \text{False Positive Rate}$$
  * Sistem memilih $\tau^*$ pada titik yang memaksimalkan nilai $J$. Hasil perbaikan menghasilkan $\tau^* = 0.9905$ dengan $F_1\text{-score} = 0.6667$, yang terbukti valid dan menolak suara satwa non-burung dengan benar.

---

### Q5: Mengapa Memakai *Similarity Retrieval* Bukan *Classifier* Biasa (Softmax)?
* **Kelemahan *Classifier* Klasik:**
  * Classifier (seperti ResNet/CNN biasa dengan lapisan Softmax) dilatih untuk menebak kelas statis: "Ini Kelas 1, 2, atau 3".
  * Butuh ribuan contoh rekaman per spesies. Jika ada spesies burung langka yang hanya punya 5 rekaman, classifier akan gagal (*overfitting* parah).
  * Jika ada spesies baru yang belum pernah dilatih, classifier **terpaksa memilih salah satu kelas yang ada** (pasti salah).
* **Keunggulan *Similarity Retrieval* (Metode TA Ini):**
  * Model hanya berfungsi sebagai "alat ukur kemiripan" (*Feature Extractor*).
  * Ketika ada query audio masuk, model mengubah audio menjadi vektor embedding, lalu menghitung jarak kosinus terhadap seluruh audio di Galeri:
    $$\text{Cosine Similarity}(u, v) = \frac{u \cdot v}{\|u\|_2 \|v\|_2}$$
  * Jika skor kemiripan tertinggi melewati ambang batas $\tau^*$, sistem mengembalikan spesies paling mirip. Jika di bawah $\tau^*$, sistem dengan cerdas menyatakan: *"Suara ini tidak dikenal / bukan spesies target"*.
  * Pendekatan ini sangat unggul untuk data terbatas (*few-shot*) dan sistem alam terbuka (*open-set*).

---

### Q6: Apa itu *Paired Stress-Testing* dan Nilai SNR (Signal-to-Noise Ratio)?
* **Arti Angka SNR (Desibel / dB):**
  * SNR mengukur perbandingan kekuatan sinyal suara burung terhadap kekuatan suara derau lingkungan.
  * **Clean:** Suara burung asli tanpa tambahan derau.
  * **20 dB:** Suara burung jauh lebih keras daripada derau (derau terdengar sangat samar).
  * **10 dB:** Suara burung jelas, tetapi derau terdengar nyata di latar belakang.
  * **0 dB:** Kekuatan suara burung persis sama kuatnya dengan kekuatan suara derau.
  * **-5 dB:** Suara derau **lebih keras** daripada suara kicauan burung (kondisi sangat bising/ekstrem).
* **Arti "Paired" (Berpasangan):**
  * Jika kita menguji audio burung A di kondisi Clean, maka di kondisi 20 dB, 10 dB, 0 dB, dan -5 dB yang diuji adalah **audio burung A yang sama persis**, hanya ditambahkan derau dengan volume terukur.
  * Ini memastikan perbandingan performa 100% adil dan objektif, bukan karena perbedaan sampel audio yang beruntung atau sial.
