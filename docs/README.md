...
| :--- | :--- | :---: | :--- |
| **Minggu 1: Hari 1-2** | • Bekukan target 10-20 burung<br>• Manifest Xeno-Canto & lisensi<br>• Bekukan sample rate & durasi | **SELESAI** | • Manifes 16 spesies (416 audio): [`data/manifests/target_birds_manifest.csv`](../data/manifests/target_birds_manifest.csv)<br>• Konfigurasi audio (32 kHz, 5.0 detik): [`configs/audio.yaml`](../configs/audio.yaml) |
| **Minggu 1: Hari 3-4** | • Implementasi MFCC + Cosine<br>• Smoke test PANNs & Bioacoustic<br>• Pilih checkpoint representasi | **SELESAI** | • Ekstraktor Model Asli: [`src/embeddings.py`](../src/embeddings.py) (BirdNET 1024-d, PANNs 2048-d)<br>• Skrip uji integtitas: [`run_tests.py`](../run_tests.py) (**9/9 Lolos 100%**) |
| **Minggu 1: Hari 5-7** | • Ambil/kurasi background ITERA<br>• Split gallery/query/calibration<br>• Buat baseline clean retrieval | **SELESAI (Split Bebas Leakage)** | • Protokol rekam ITERA: [`docs/protocols/itera-recording.md`](protocols/itera-recording.md)<br>• Manifes split: [`data/manifests/dataset_split.csv`](../data/manifests/dataset_split.csv)<br>• Baseline retrieval: [`results/processed/snr_robustness_table.csv`](../results/processed/snr_robustness_table.csv) (Kolom `Clean`) |
| **GATE MINGGU 1** | **Minimal 3 representasi (termasuk MFCC) menghasilkan embedding & pipeline bebas leakage** | **Kriteria Terpenuhi (Siap Ditinjau)** | **[`run_tests.py`](../run_tests.py) lolos 9/9 unit test; Irisan perekam Galeri vs Query tepat 0 (Strict Zero Leakage)** |
| **Minggu 2: Hari 8-10**| • Implementasi mixing SNR reproducible<br>• Pilot level SNR (20 s/d -5 dB)<br>• Bekukan noise segment & seed | **SELESAI** | • Modul derau deterministik (seed 42): [`src/mix_noise.py`](../src/mix_noise.py)<br>• Definisi level SNR: [`configs/audio.yaml`](../configs/audio.yaml) |
| **Minggu 2: Hari 11-14**| • Jalankan E1-E2 seluruh query<br>• Simpan ranking per query<br>• Buat robustness curve awal | **SELESAI** | • Tabel hasil E2: [`results/processed/snr_robustness_table.csv`](../results/processed/snr_robustness_table.csv)<br>• Grafik publikasi mAP@10 (95% CI): [`results/figures/robustness_curve_map10.png`](../results/figures/robustness_curve_map10.png)<br>• 20 Log per-kueri: [`results/raw/`](../results/raw/) |
| **GATE MINGGU 2** | **Tidak ada ceiling/floor total, query pairs konsisten, kontrol acak masuk akal** | **Kriteria Terpenuhi (Siap Ditinjau)** | **Nilai mAP terdistribusi alami (0.58 -> 0.47 pada $R_2$), Kontrol acak ($R_3$) stabil di 0.020** |
| **Minggu 3: Hari 15-18**| • Susun unknown/background set<br>• Kalibrasi threshold ($\tau$) pada calibration split<br>• Jalankan open-set test semua SNR | **SELESAI** | • Dataset unknown (77 audio): [`data/manifests/unknown_open_set_manifest.csv`](../data/manifests/unknown_open_set_manifest.csv)<br>• Kalibrasi ROC Youden's J: [`src/evaluate.py`](../src/evaluate.py) ($\tau^* = 0.6488$ pada $R_2$)<br>• Tabel open-set dinamis: [`results/processed/threshold_transfer_table.csv`](../results/processed/threshold_transfer_table.csv) |
| **Minggu 3: Hari 19-21**| • Anotasi subset real soundscape ITERA<br>• Validasi eksternal tanpa re-tuning<br>• Audit minimal 20 failure cases | **Audit E5 Selesai (30 Kasus Riil) — ITERA Ditunda** | • Audit kegagalan retrieval riil (30 kasus): [`results/processed/failure_analysis_table.csv`](../results/processed/failure_analysis_table.csv)<br>• *Catatan:* Pengambilan audio fisik di ITERA ditunda sebagai tahap lanjutan (D-03) |
| **Minggu 4: Hari 22-30**| • Paired bootstrap CI & kurva final<br>• Naskah laporan & artikel v0.8<br>• Freeze code & materi bimbingan | **SELESAI (Notebook Terpatri)** | • Notebook interaktif resmi: [`notebooks/01_evaluasi_benchmark_dan_visualisasi.ipynb`](../notebooks/01_evaluasi_benchmark_dan_visualisasi.ipynb)<br>• Catatan progres: [`Rencana-eksperimen-bimbingan/CATATAN_PROGRES_BIMBINGAN.md`](../Rencana-eksperimen-bimbingan/CATATAN_PROGRES_BIMBINGAN.md) |
